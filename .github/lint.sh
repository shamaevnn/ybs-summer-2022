...
: '
  Usage:
  `./.github/lint.sh <command> <path-to-code-directory>`,
...
case ${COMMAND} in
    ...
    check-flake8)
        flake8 --config ./setup.cfg ${FILEPATH}
    ...
    install)
        ...
        ;;
    *)
        echo $"Usage: $SCRIPT_NAME {check|check-black|check-isort|check-flake8|check-mypy|diff-isort|diff-black|format|install}"
        exit 1
esac
